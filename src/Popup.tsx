...
export default function Popup() {
	...
	const computeAlerts = (
		...
		if (daily > dLimit * 0.9)
			arr.push(
				`Daily limit almost reached: ${((daily / dLimit) * 100).toFixed(0)}%`,
			);
		if (weekly > wLimit * 0.9)
			arr.push(
				`Weekly limit almost reached: ${((weekly / wLimit) * 100).toFixed(0)}%`,
			);
		if (monthly > mLimit * 0.9)
			arr.push(
				`Monthly limit almost reached: ${((monthly / mLimit) * 100).toFixed(0)}%`,
			);
		setAlerts(arr);
	};
